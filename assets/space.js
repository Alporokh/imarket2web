...
(function(){
  ...
  // Beat thresholds, as fractions of the journey. The short variant is not a
  ...
  function at(k,p){return smooth(T[k][0],T[k][1],p)}
  ...
  function draw(p){ctx.clearRect(0,0,w,h);ctx.fillStyle='#020306';ctx.fillRect(0,0,w,h);var cx=w*.5,cy=h*.48;var warp=at('warp',p), fade=1-at('fade',p);ctx.globalAlpha=fade;stars.forEach(function(st){var boost=1+p*1.5, x=cx+st.x*boost,y=cy+st.y*boost;var len=warp*(18+st.z*115);var dx=x-cx,dy=y-cy,mag=Math.sqrt(dx*dx+dy*dy)||1;ctx.beginPath();ctx.moveTo(x-dx/mag*len,y-dy/mag*len);ctx.lineTo(x,y);ctx.strokeStyle='rgba(190,218,255,'+(.28+st.z*.7)+')';ctx.lineWidth=st.s*(.65+warp*1.1);ctx.stroke();if(warp<.35){ctx.beginPath();ctx.arc(x,y,st.s,0,Math.PI*2);ctx.fillStyle='rgba(255,255,255,'+(.35+st.z*.55)+')';ctx.fill()}});ctx.globalAlpha=1;
    var copyOut=at('copyOut',p);copy.style.opacity=String(1-copyOut);copy.style.transform='translate3d(0,'+(-70*copyOut)+'px,0) scale('+(1-.035*copyOut)+')';
    hint.style.opacity=String(.8*(1-at('hint',p)));
    var earthIn=at('earthIn',p),earthGrow=at('earthGrow',p);earth.style.opacity=String(earthIn);var scale=.055+earthGrow*1.55;earth.style.transform='translate3d(0,'+(18-20*earthGrow)+'%,0) scale('+scale+')';
    var msgIn=at('msgIn',p),msgOut=at('msgOut',p);message.style.opacity=String(msgIn*(1-msgOut));message.style.transform='translate(-50%,'+(-42-8*msgIn)+'%) scale('+(1+.04*msgIn)+')';
  }
